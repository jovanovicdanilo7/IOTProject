...
void main(void)
{
  ...
  client_addr.sin_port = htons(PORT_NUM1);
  ...
  if( (retcode = recvfrom(multi_server_sock, buffer1, sizeof(buffer1), 0,
  ...
  sprintf(buffer3,"RESPONSE * HTTP/1.1 200 OK\r\n"
		  ...
		  "SERVER: LINUX\r\n"
		  "ST: senzor_pritiska\r\n"
		  "USN: uuid:12345678-90ab-cdef-1234-567890abcdef::urn:schemas-upnp-org:device:senzor_pritiska\r\n"
		  "\r\n", day, month, year, GROUP_ADDR, PORT_NUM1);
  ...
  char pritisak[10];
  while (1) {
      printf("Enter pressure value (or 'q' to quit): ");
      fgets(pritisak, sizeof(pritisak), stdin);
      ...
      // Publish pritisak
      mosquitto_publish(mosq, NULL, "korisnik/pritisak", strlen(pritisak), pritisak, 0, false);
  }
